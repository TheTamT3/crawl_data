...
class Article:
    def __init__(self, article_id,href, topic, date, newspaper, language, sapo, content,  avatar, post_type, author_id=None, author_fullname = '', tags=[]):
        self._id = article_id
        self._href=href
...
class ArticleManager:
        ...
        if (result is not None): # found an article
            if result != False:
                ...
                if self.is_not_outdated(newsdate) or webconfig.get_skip_crawl_publish_date():
                    ...
                    return ((topic, newsdate, sapo, content, avatar_url), has_visit) #, feature_image
                else:
                                ...
                                if ('href' in id_type) and (filter.match(fullurl) is None):
                                    ...
                                    if result is not None: # no errors happend
                                        if result != False: # valid url
                                            (topic, publish_date, sapo, content , avatar_url) = result #, /*feature_image*/
                                            if topic[-4:] == 'icon':
                                                topic = topic[:-4]
                                            if topic[-1:] == '.':
                                                topic = topic[:-1]
                                            ...
                                            new_article = Article(article_id=next_id,
                                                                     ...
                                                                     author_fullname=webname,
                                                                     tags = tags)
                                            if only_quality_post: #feature_image=feature_image,
    ...
    def get_articles_as_json(self, number=None):
        ...
        for index in range(0, number):
            ...
            json_article_list.append({#'stt':str(count),'content': 
                                      content:''.join(article.get_content())
    ...
    def export_to_json(self, number=None):
        ...
        with open_utf8_file_to_write(get_independent_os_path(["export", "article_data.json"])) as stream:
            json.dump({'article_list': json_article_list}, stream,ensure_ascii=False)
        print("OK")
